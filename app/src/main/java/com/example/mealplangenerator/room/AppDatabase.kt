        ...
        override fun onCreate(db: SupportSQLiteDatabase) {
            ...
            INSTANCE?.let { database ->
                scope.launch {
                    ...
                    insertSideDishes(database)
                }
            }
        }

        private suspend fun insertSideDishes(database: AppDatabase) {
            val sideDishDao = database.sideDishDao() ?: return

            sideDishDao.deleteAll()

            sideDishDao.insertAll(
                SideDish("Champignon"),
                SideDish("Ratatouille"),
                SideDish("Riz"),
                SideDish("Pates"),
                SideDish("Haricots vert"),
                SideDish("Carrote"),
                SideDish("Patate (frite / potatoes)"),
                SideDish("Purée"),
                SideDish("Haricot blanc à la tomate"),
                SideDish("Quinoa"),
                SideDish("Choufleur"),
                SideDish("Petit pois / carottes"),
            )
        }

        private suspend fun insertMainDishes(database: AppDatabase) {
            val mainDishDao = database.mainDishDao() ?: return
            ...
            with(mainDishDao) {
                ...
                insertAll(
                        ...
                        MainDish("Pavé de saumon", MealTime.LUNCH, Duration.QUICK, 2, "",
                            isStaple = true,
                            isFullCourse = true
                        ),
                        MainDish(
                        ...
                        MainDish("Croquette de haddock", MealTime.ANY, Duration.MEDIUM, 1, "",
                            isStaple = false,
                            isFullCourse = true
                        ),
